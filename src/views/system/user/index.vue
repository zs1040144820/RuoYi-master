          ...
          <el-col :span="12">
            <el-form-item v-if="form.userId == undefined" label="用户密码" prop="password">
              ...
              <div class="intensity">
                ...
                <!--<div class="warningtext" style="color: red;">应由8-20位数字、字母、符号组成</div>-->
              </div>
            </el-form-item>
          </el-col>
...
export default {
  ...
  data() {
    return {
      ...
      rules: {
        ...
        password: [
          { required: true, message: "密码强度过低，应为数字、字母和符号的组合，长度为8到20位", validator: this.checkPassword, trigger: 'change' },
        ],
        email: [
          {
            type: "email",
  ...
  methods: {
    ...
    handleResetPwd(row) {
      this.$prompt('请输入"' + row.userName + '"的新密码', "提示", {
        ...
        inputErrorMessage: "用户密码长度必须介于 8 和 20 之间"
          ...
          // 校验密码强度
          if (!this.checkPwdStrength(value)) {
            this.$message.error("密码必须包含字母、数字和字符中至少两种，并且长度必须在8-20位之间。");
            return;
          }
    ...
    checkPassword(rule, value, callback) {
      ...
      if (this.level.includes('low')) {
        return callback('密码强度过低，密码应为数字、字母和符号的组合');
      }
      return callback()
    },
  }
